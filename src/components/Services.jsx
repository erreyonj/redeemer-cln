...
const Services = (isPage) => {
    ...
    const pageClass = 'h-[450px] grid place-content-space-between lg:grid-cols-50-50'
    const notPageClass = 'h-[450px] flex flex-row place-content-center' 
  ...
  return (
    <section id="sectionServices" className={currentPage==='Services'?pageClass:notPageClass}>
        ...
        <div id="swipeContainer" className={`p-3 h-96 md:h-[520px] lg:w-[1830px] ${(currentPage==='Services')&&first()} overflow-hidden`}>
            <div id="serviceCards" className={`h-[350px] md:w-[80] lg:w-[675px] md:h-[400px] py-5 ${isPage?'':''} gap-5 flex flex-row place-content-between lg:flex-nowrap overflow-hidden snap-x *:mx-5  md:h-auto `}> {/* <-- maybe add shadow around cards here? shadow-slate-300 shadow-inner*/}
